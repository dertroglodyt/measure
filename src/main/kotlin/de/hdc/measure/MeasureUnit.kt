...
class COMBINED(q: Quantity) :
  MeasureUnit<SI_COMBINED>(NONE, "combined", q.toString(), SI_COMBINED(q))
...
object g : MeasureUnit<SI_KILOGRAM>(
  NONE, "Gram", "g", SI_KILOGRAM,
...
object h : MeasureUnit<SI_SECOND>(
  NONE, "Hour", "h", SI_SECOND,
  multiplier = context(4).valueOf(3600.0)
)

object d : MeasureUnit<SI_SECOND>(
  NONE, "Day", "d", SI_SECOND,
  multiplier = context(5).valueOf(24.0 * 3600.0)
)

object AU : MeasureUnit<SI_METER>(
  NONE, "Astronomical Unit", "AU", SI_METER,
  multiplier = context(12).valueOf(149597870700)
)

object ly : MeasureUnit<SI_METER>(
  NONE, "Light Year", "ly", SI_METER,
  multiplier = context(16).valueOf(9460730472580800)
)

object pc : MeasureUnit<SI_METER>(
  NONE, "Parsec", "pc", SI_METER,
  multiplier = context(16).valueOf(3.08567758149137e16)
)

object L : MeasureUnit<SI_VOLUME>(
  NONE, "Litre", "L", SI_VOLUME,
  multiplier = context(3).valueOf(1e-3)
)

//object kg : MeasureUnit<SI_KILOGRAM>(Prefix.k, "Kilogram", "kg", SI_KILOGRAM)
object t : MeasureUnit<SI_KILOGRAM>(
  NONE, "Ton", "t", SI_KILOGRAM,
  multiplier = context(3).valueOf(1e3)
)

object `°C` : MeasureUnit<SI_KELVIN>(
  NONE, "Celsius", "°C", SI_KELVIN,
  increment = context(5).valueOf(-273.15)
)

object m_s : MeasureUnit<SI_VELOCITY>(NONE, "Velocity", "m/s", SI_VELOCITY)
object km_h : MeasureUnit<SI_VELOCITY>(
  NONE, "Velocity", "km/h", SI_VELOCITY,
  multiplier = context(2).valueOf(3.6)
)

object m_s2 : MeasureUnit<SI_ACCELERATION>(NONE, "Acceleration", "m/s²", SI_ACCELERATION)
object `°` : MeasureUnit<SI_RADIAN>(
  NONE, "Degree", "°", SI_RADIAN,
  multiplier = Measure.defaultContext.valueOf(Math.PI / 180.0)
)

object rad_s :
  MeasureUnit<SI_ANGULAR_VELOCITY>(NONE, "Angular Velocity", "rad/s", SI_ANGULAR_VELOCITY)
