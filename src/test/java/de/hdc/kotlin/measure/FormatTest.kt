...
class FormatTest {
  ...
  fun main() {
    ...
    assertEquals("2300,000 Yg", pretty(Measure(2300000.0, MeasureUnit(ZETA, DU_GRAM))))
    // Default values: 4 padding, 3 digits, auto-stepup overflow
    assertEquals("2300,000 Yg", pretty(Measure(2300.0, MeasureUnit(YOTA, DU_GRAM))))
    // Default values: 4 padding, 3 digits, auto-stepup overflow, padding overflow
    assertEquals("23000,000 Yg", pretty(Measure(23000.0, MeasureUnit(YOTA, DU_GRAM))))
    // Default values: 4 padding, 3 digits, auto-stepdown underflow
    assertEquals("   0,230 yg", pretty(Measure(0.23, MeasureUnit(YOCTO, DU_GRAM))))
    // Default values: 4 padding, 3 digits, auto-stepdown underflow
    assertEquals("   0,000 yg", pretty(Measure(0.00023, MeasureUnit(YOCTO, DU_GRAM))))
