...
class MeasureUnitKTest: FreeSpec() {
  ...
  init {
    ...
    "Test equals()" - {
      ...
      "predefined & SI units" {
        kg.shouldBe(MeasureUnit(KILO, "", "", SI_GRAM))
      }

      "predefined & COMBINED" {
        kg.shouldNotBe(MeasureUnit(NONE, "", "", SI_COMBINED(Quantity(0, 0, 0, 0, 1, 0, 0))))
        kg.shouldBe((0.0 k COMBINED(Quantity(0, 0, 0, 0, 1, 0, 0))).unit)
        kg.shouldBe(MeasureUnit(KILO, "", "", SI_COMBINED(Quantity(0, 0, 0, 0, 1, 0, 0))))

        MeasureUnit(KILO, "", "", SI_COMBINED(Quantity(0, 0, 0, 0, 1, 0, 0)))
                .shouldBe(kg)
      ...
      "COMBINED" {
        MeasureUnit(KILO, "", "", SI_COMBINED(Quantity(0, -2, 0, 0, 1, 0, 0)))
                .shouldBe(MeasureUnit(KILO, "", "", SI_COMBINED(Quantity(0, -2, 0, 0, 1, 0, 0))))
      }
    }
